...
export default function KeywordSearch() {
  ...
  const handleSearch = async (query: string) => {
    ...
    setLoading(true);
    setDisplayLimit(5); // Reset display limit for new search
    try {
    ...
    <div className="w-full">
      ...
      <div className="relative">
        <input
          ...
          placeholder="Type a keyword (e.g. labour law, industrial relations)"
            className="w-full px-6 py-4 text-lg border-2 border-slate-200 dark:border-gray-600 rounded-xl focus:outline-none focus:border-blue-500 dark:focus:border-blue-400 bg-white dark:bg-gray-700 text-slate-900 dark:text-white placeholder-slate-500 dark:placeholder-gray-400 shadow-sm"
        />
        <button
            onClick={() => handleSearch(searchQuery)}
          className="absolute right-2 top-2 bottom-2 px-6 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors flex items-center"
        >
          ...
          {searchResults.length > 0 ? (
              ...
              {searchResults.filter(scholar => scholar.frequentContributor).length > 0 && (
                <div>
                  ...
                  <div className="space-y-4">
                    {searchResults.filter(scholar => scholar.frequentContributor).slice(0, displayLimit).map((scholar) => (
                      <div
                        key={scholar._id}
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {scholar.keywordIds.slice(0, 6).map((keyword, index) => (
                                <span
                                  key={index}
                              ...
                              {scholar.keywordIds.length > 6 && (
                                <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs rounded-full">
                                  +{scholar.keywordIds.length - 6} more
                                </span>
                              )}
                            </div>
                          </div>
                  ...
                  {searchResults.filter(scholar => scholar.frequentContributor).length > displayLimit && (
                    <div className="mt-4 text-center">
                      <button
                        onClick={handleShowMore}
                        className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium"
                      >
                        Show More ({searchResults.filter(scholar => scholar.frequentContributor).length - displayLimit} more)
                      </button>
                    </div>
                  )}
                  {displayLimit > 5 && searchResults.filter(scholar => scholar.frequentContributor).length > 5 && (
                    <div className="mt-2 text-center">
                      <button
                        onClick={handleShowLess}
                        className="px-4 py-1 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 text-sm font-medium"
                      >
                        Show Less
                      </button>
                    </div>
                ...
                <div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between mb-3">
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <Link
                                  href={`/occasional-contributors#${scholar.slug}`}
                                  className="text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 text-sm font-medium"
                                >
                                  View Profile →
                                </Link>
                  ...
                  {displayLimit > 5 && searchResults.filter(scholar => !scholar.frequentContributor).length > 5 && (
                    <div className="mt-2 text-center">
                      <button
                        onClick={handleShowLess}
                        className="px-4 py-1 text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 text-sm font-medium"
                      >
                        Show Less
                      </button>
                    </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-600 dark:text-gray-300 mb-6">
                No scholars found with the keyword &quot;{searchQuery}&quot;. Try searching with different keywords or check our{" "}
