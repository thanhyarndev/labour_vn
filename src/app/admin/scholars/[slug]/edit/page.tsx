...
import { IScholar, CreateScholarInput, CreatePublicationInput, IPublication, CreateKeywordInput, IKeyword } from "@/models";
...
export default function EditScholarPage() {
  ...
  const [scholar, setScholar] = useState<IScholar | null>(null);
  ...
  useEffect(() => {
    const fetchScholar = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/admin/scholars?slug=${slug}`);
        const data = await response.json();
        
        if (data.success && data.data.length > 0) {
          const scholarData = data.data[0];
          setScholar(scholarData);
          
          // Parse name from fullName
          const nameParts = scholarData.fullName.split(' ');
          ...
          setFormData({
            ...
            lastName,
            middleName,
            title: scholarData.title || "", // Get from scholarData
            position: scholarData.position || "", // Get from scholarData
            institution: scholarData.affiliation || "",
          ...
          setKeywords(scholarData.keywordIds || []);
          setPublications(scholarData.publicationIds || []);
        } else {
          showError("Scholar not found", "The requested scholar could not be found");
          router.push("/admin/scholars");
        }
      } catch (error) {
        console.error("Error fetching scholar:", error);
        showError("Error loading scholar data", "Failed to load scholar information");
        router.push("/admin/scholars");
      } finally {
        setLoading(false);
      }
  ...
  // Load existing slugs for validation
  useEffect(() => {
    const fetchExistingSlugs = async () => {
      try {
        const response = await fetch('/api/admin/scholars?limit=1000');
        const data = await response.json();
        if (data.success) {
          const slugs = data.data
            .filter((s: IScholar) => s.slug !== slug) // Exclude current scholar
            .map((s: IScholar) => s.slug);
  ...
  useEffect(() => {
    if (!isManualSlug && scholar) {
      const fullName = `${formData.firstName} ${formData.middleName} ${formData.lastName}`.trim();
      if (fullName) {
        const newSlug = generateUniqueSlug(fullName, existingSlugs);
        setFormData(prev => ({
          ...prev,
          slug: newSlug
        }));
  ...
  }, [formData.firstName, formData.middleName, formData.lastName, existingSlugs, isManualSlug, scholar]);
  ...
  const handleCreatePublication = async (publicationData: CreatePublicationInput) => {
    try {
      ...
      if (response.ok) {
        ...
        showSuccess('Publication created successfully!', 'The publication has been added to the scholar');
      } else {
        showError('Failed to create publication', 'Could not create the publication');
      }
    } catch (error) {
      console.error('Failed to create publication:', error);
      showError('Failed to create publication', 'An error occurred while creating the publication');
    } finally {
  ...
  const handleLinkPublication = async (publicationIds: string[]) => {
    try {
      setPublicationLoading(true);
      // Fetch all selected publications
      const responses = await Promise.all(
        publicationIds.map(id => fetch(`/api/admin/publications/${id}`))
      );
      ...
      // Add all successfully fetched publications to the list
      const newPublications = results
        .filter(result => result.success)
        .map(result => result.data);
      
      setPublications(prev => [...prev, ...newPublications]);
      setShowLinkPublicationModal(false);
      showSuccess(`${newPublications.length} publication${newPublications.length !== 1 ? 's' : ''} linked successfully!`, 'Publications have been linked to the scholar');
    } catch (error) {
  ...
  const handleCreateKeyword = async (keywordData: CreateKeywordInput) => {
    try {
      ...
      if (response.ok) {
        ...
        showSuccess('Keyword created successfully!', 'The keyword has been added to the scholar');
      } else {
        showError('Failed to create keyword', 'Could not create the keyword');
      }
    } catch (error) {
      console.error('Failed to create keyword:', error);
      showError('Failed to create keyword', 'An error occurred while creating the keyword');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const scholarData = {
        ...formData,
        slug: manualSlug,
        keywordSlugs: keywords.map(k => k.slug),
        newKeywords: [], // For now, we'll handle keywords separately
      ...
      // Update scholar via API
      const response = await fetch(`/api/admin/scholars/${scholar._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (!response.ok) {
        throw new Error('Failed to update scholar');
      }

      const result = await response.json();
      console.log("Scholar updated:", result);
      
      showSuccess("Scholar updated successfully!", "The scholar information has been saved");
      router.push("/admin/scholars");
    } catch (error) {
      console.error("Failed to update scholar:", error);
      showError("Failed to update scholar", "Could not save the scholar information");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        <p className="ml-2 text-gray-600 dark:text-gray-400">Loading scholar data...</p>
      </div>
    );
  }
  ...
  if (!scholar) {
    return (
      <div className="text-center py-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Scholar not found</h1>
        <Link href="/admin/scholars" className="text-blue-600 hover:text-blue-800">
          ← Back to Scholars
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex space-x-2">
              ...
              <button
                type="button"
                onClick={() => setShowLinkPublicationModal(true)}
      ...
      <LinkPublicationModal
        ...
        onClose={() => setShowLinkPublicationModal(false)}
      ...
      <LinkKeywordModal
        ...
        onClose={() => setShowLinkKeywordModal(false)}
